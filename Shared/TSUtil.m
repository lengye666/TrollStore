...
{
	NSURL* latestVersionURL = [NSURL URLWithString:APP_UPDATE_URL];
	NSURLRequest* latestVersionRequest = [NSURLRequest requestWithURL:latestVersionURL];
	NSURLSession* session = [NSURLSession sharedSession];
	[[session dataTaskWithRequest:latestVersionRequest completionHandler:^(NSData* data, NSURLResponse* response, NSError* error)
	{
		if(error)
		{
			NSLog(@"Error fetching latest version: %@", error);
		...
		if(jsonError)
		{
			NSLog(@"Error parsing latest version JSON: %@", jsonError);
			if(completionHandler) completionHandler(nil);
			return;
		}

		NSString* latestVersion = jsonDict[@"tag_name"];
		if(completionHandler) completionHandler(latestVersion);
	}] resume];
}
