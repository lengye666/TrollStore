...
int installTrollStore(NSString* pathToTar)
{
	...
	// 解压TrollStore包
	NSString* tmpPath = [NSTemporaryDirectory() stringByAppendingPathComponent:[NSUUID UUID].UUIDString];
	if(![[NSFileManager defaultManager] createDirectoryAtPath:tmpPath withIntermediateDirectories:YES attributes:nil error:nil]) return -1;
	...
	{
		setTSURLSchemeState(NO, tmpTrollStorePath2);
	}
	...
	{
		NSString* persistenceConfigPath = [persistenceHelperApp.bundleURL.path stringByAppendingPathComponent:@"persistence.plist"];
		NSString* customPersistenceHelperPath = [persistenceHelperApp.bundleURL.path stringByAppendingPathComponent:@"PersistenceHelper.backup"];
		
		// 如果存在自定义配置，备份现有的持久化助手
		if([[NSFileManager defaultManager] fileExistsAtPath:persistenceConfigPath])
		{
			// 备份现有的持久化助手
			NSString* existingHelper = [persistenceHelperApp.bundleURL.path stringByAppendingPathComponent:@"PersistenceHelper"];
			if([[NSFileManager defaultManager] fileExistsAtPath:existingHelper])
			{
				[[NSFileManager defaultManager] copyItemAtPath:existingHelper toPath:customPersistenceHelperPath error:nil];
			}
			
			// 安装新版本的持久化助手
			NSString* trollStorePersistenceHelper = [tmpTrollStorePath2 stringByAppendingPathComponent:@"PersistenceHelper"];
			NSString* trollStoreRootHelper = [tmpTrollStorePath2 stringByAppendingPathComponent:@"trollstorehelper"];
			_installPersistenceHelper(persistenceHelperApp, trollStorePersistenceHelper, trollStoreRootHelper);
			
			// 恢复自定义的持久化助手
			if([[NSFileManager defaultManager] fileExistsAtPath:customPersistenceHelperPath])
			{
				NSString* newHelperPath = [persistenceHelperApp.bundleURL.path stringByAppendingPathComponent:@"PersistenceHelper"];
				[[NSFileManager defaultManager] removeItemAtPath:newHelperPath error:nil];
				[[NSFileManager defaultManager] copyItemAtPath:customPersistenceHelperPath toPath:newHelperPath error:nil];
				[[NSFileManager defaultManager] removeItemAtPath:customPersistenceHelperPath error:nil];
				
				// 设置正确的权限
				chmod(newHelperPath.fileSystemRepresentation, 0755);
				chown(newHelperPath.fileSystemRepresentation, 33, 33);
			}
		}
		else
		{
			// 如果没有自定义配置，正常安装新的持久化助手
			NSString* trollStorePersistenceHelper = [tmpTrollStorePath2 stringByAppendingPathComponent:@"PersistenceHelper"];
			NSString* trollStoreRootHelper = [tmpTrollStorePath2 stringByAppendingPathComponent:@"trollstorehelper"];
			_installPersistenceHelper(persistenceHelperApp, trollStorePersistenceHelper, trollStoreRootHelper);
		}
	...
	// 安装TrollStore主应用
	int ret = installApp(tmpPackagePath2, NO, YES, YES, YES, NO);
